...
cv.addEventListener('pointerdown',e=>{const p=toDesign(e);AudioUnlock();G.pdown={x:p.x,y:p.y,moved:false,t:now()};
  ...
  // during capsule/flash phases (0,1) taps just skip the animation; in the reveal phase (2) fall through so the OK button is clickable
  if(G.gachaAnim&&G.gachaAnim.phase<2){G.gachaAnim.tap=true;return}
  for(let i=G.hits.length-1;i>=0;i--){const h=G.hits[i];if(h.hidden)continue;
    if(p.x>=h.x&&p.x<=h.x+h.w&&p.y>=h.y&&p.y<=h.y+h.h){if(h.scroll){G.dragScroll={h,off:h.off(),sy:p.y,sx:p.x,moved:false};return}
      if(h.cb){G.pend={h,p};if(!h.noactive)h.active=true}return}}
});
...
function endPointer(e){if(G.dragScroll){if(!G.dragScroll.moved&&G.dragScroll.h.tap){G.dragScroll.h.tap()}G.dragScroll=null}
...
function drawMap(dt){const c=CHMAP[G.chapter];
  ...
  BTN('attack',986,604,278,68,()=>{
    if(!canPlay){toast('Stage locked — clear the previous one first!','#ffb060');SFX.error();return}
